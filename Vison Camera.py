                ...
                try:
                    ...
                    if cat == "생산" and not df_bom.empty:
                        ...
                        if '타입' in df_bom.columns:
                            bom_targets = bom_targets[df_bom['타입'].astype(str) == str(sel_type)]
            ...
            if delete_options:
                ...
                with col_act1:
                    if st.button("🗑️ 선택한 기록 삭제 (원자재 재고 복구)", type="primary"):
                        ...
                        # 연동된 사용(Auto) 로그 찾아서 원자재 재고 복구
                        linked_logs = df_logs[
                            (df_logs['날짜'] == del_date) &
                            (df_logs['시간'] == del_time) &
                            (df_logs['구분'] == '사용(Auto)') &
                            (df_logs['비고'].astype(str).str.contains(str(del_code), na=False))
                        ]
                        rows_to_delete = [sel_target_id]
                        for idx, row in linked_logs.iterrows():
                            mat_qty = safe_float(row['수량'])  # 음수 저장됨 (-req)
                            # 원자재 복구: 음수를 다시 빼면 양수 = 재고 증가
                ...
                if st.session_state["edit_mode"]:
                    ...
                    with st.form("edit_form"):
                        ...
                        if st.form_submit_button("✅ 수정사항 저장"):
                            ...
                            # 기존 원자재 사용 복구
                            linked_logs_old = df_logs[
                                (df_logs['날짜'] == old_date) &
                                (df_logs['시간'] == old_time) &
                                (df_logs['구분'] == '사용(Auto)') &
                                (df_logs['비고'].astype(str).str.contains(str(old_code), na=False))
                            ]
                ...
                if st.button("🚀 출고 확정", type="primary"):
                    for _, row in d_out.iterrows():
                        ...
                        sheet_logs.append_row([
                            ...
                            row['코드'], row['품목명'], "-", "-", "-",
                            -safe_float(row['수량']), f"주문출고({tgt_out})", row['거래처'], "-"
                        ])
                        time.sleep(0.2)
        ...
        if not df_orders.empty and '상태' in df_orders.columns:
            ...
            if not done.empty:
                ...
                if st.button("↩️ 출고 취소 실행 (재고 복구)", type="primary"):
                    ...
                    # 2. 출고 로그에서 해당 주문 로그 제거
                    if not df_logs.empty:
                        cancel_log_rows = df_logs[
                            (df_logs['구분'] == '출고') &
                            (df_logs['비고'].astype(str).str.contains(str(tgt_cancel), na=False))
                        ]
                        # 역순으로 삭제 (행 인덱스 밀림 방지)
                        del_indices = sorted([idx + 2 for idx in cancel_log_rows.index], reverse=True)
...
# ==================== [3] 현장 작업 (LOT 입력) — 버그수정: 미구현 → 구현 ====================
elif menu == "🏭 현장 작업 (LOT 입력)":
    st.title("🏭 현장 작업 입력 (간편 LOT)")
    st.info("💡 현장 작업자용 간편 입력 화면입니다. 품목 코드를 직접 입력하거나 선택하여 빠르게 기록합니다.")

    with st.form("lot_form", clear_on_submit=True):
        c1, c2, c3 = st.columns(3)
        lot_date    = c1.date_input("작업일", datetime.date.today())
        lot_factory = c2.selectbox("공장", ["1공장", "2공장"])
        lot_cat     = c3.selectbox("구분", ["생산", "입고"])

        line_options_lot = []
        if lot_factory == "1공장":   line_options_lot = [f"압출{i}호" for i in range(1, 6)] + ["기타"]
        elif lot_factory == "2공장": line_options_lot = [f"압출{i}호" for i in range(1, 7)] + [f"컷팅{i}호" for i in range(1, 11)] + ["기타"]

        c4, c5 = st.columns(2)
        lot_line = c4.selectbox("설비 라인", line_options_lot) if lot_cat == "생산" else "-"

        # 품목 선택
        if not df_items.empty:
            df_lot_items = df_items.copy()
            if lot_cat == "생산": df_lot_items = df_lot_items[df_lot_items['구분'].isin(['제품', '완제품', '반제품'])]
            else: df_lot_items = df_lot_items[df_lot_items['구분'] == '원자재']

            df_lot_items['Disp'] = df_lot_items['코드'].astype(str) + " | " + df_lot_items['품목명'].astype(str) + " (" + df_lot_items['규격'].astype(str) + ")"
            lot_sel = c5.selectbox("품목 선택", df_lot_items['Disp'].unique())
            lot_row = df_lot_items[df_lot_items['Disp'] == lot_sel].iloc[0]
        else:
            st.warning("품목 데이터가 없습니다.")
            lot_row = None

        c6, c7 = st.columns(2)
        lot_qty  = c6.number_input("수량 (kg)", min_value=0.0, step=10.0)
        lot_note = c7.text_input("비고 (LOT번호 등)")

        submitted = st.form_submit_button("✅ 작업 기록 저장", type="primary")

    if submitted and lot_row is not None:
        if lot_qty <= 0:
            st.error("🚨 수량을 입력해주세요.")
        elif sheet_logs:
            try:
                ...
                sheet_logs.append_row([
                    ...
                    lot_row['코드'], lot_row['품목명'], lot_row['규격'],
                    lot_row['타입'], lot_row['색상'], lot_qty, lot_note, "-", lot_line
                ])
                update_inventory(lot_factory, lot_row['코드'], lot_qty,
                                 lot_row['품목명'], lot_row['규격'],
                                 lot_row['타입'], lot_row['색상'])

                if lot_cat == "생산" and not df_bom.empty:
                    bom_targets = df_bom[df_bom['제품코드'].astype(str) == str(lot_row['코드'])]
                    if '타입' in df_bom.columns:
                        bom_targets = bom_targets[bom_targets['타입'].astype(str) == str(lot_row['타입'])]
                    bom_targets = bom_targets.drop_duplicates(subset=['자재코드'])
                    for _, r in bom_targets.iterrows():
                        ...
                        sheet_logs.append_row([
                            ...
                            r['자재코드'], "System", "-", "-", "-", -req,
                            f"{lot_row['코드']} 생산", "-", lot_line
                        ])
                st.cache_data.clear()
                st.success(f"✅ [{lot_row['코드']}] {lot_cat} {lot_qty:,.0f}kg 저장 완료!")
            except Exception as e: st.error(f"오류: {e}")
    ...
    if not df_logs.empty:
        ...
        df_today_lot = df_logs[(df_logs['날짜'] == today_str) & (df_logs['구분'].isin(['생산', '입고']))].copy()
        if not df_today_lot.empty:
            safe_disp = [c for c in ['날짜', '시간', '공장', '구분', '코드', '품목명', '수량', '비고'] if c in df_today_lot.columns]
            st.dataframe(df_today_lot[safe_disp].sort_values('시간', ascending=False), use_container_width=True, hide_index=True)
            st.metric("오늘 총 생산량", f"{df_today_lot[df_today_lot['구분']=='생산']['수량'].sum():,.0f} kg")
        else: st.info("오늘 작업 기록이 없습니다.")
    else: st.info("데이터가 없습니다.")

# ==================== [4] 이력/LOT 검색 — 버그수정: 미구현 → 구현 ====================
elif menu == "🔍 이력/LOT 검색":
    st.title("🔍 이력 및 LOT 통합 검색")

    s1, s2, s3 = st.columns(3)
    srch_keyword = s1.text_input("🔑 키워드 검색 (코드/품목명/비고)", placeholder="예: KA100, LOT-001")
    srch_type    = s2.multiselect("구분 필터", ["생산", "입고", "출고", "사용(Auto)", "재고실사"], default=["생산", "입고", "출고"])
    srch_fac     = s3.radio("공장", ["전체", "1공장", "2공장"], horizontal=True)

    d1, d2 = st.columns(2)
    if not df_logs.empty and '날짜' in df_logs.columns:
        min_log_dt = pd.to_datetime(df_logs['날짜']).min().date()
    else:
        min_log_dt = datetime.date.today() - datetime.timedelta(days=30)
    srch_date_range = d1.date_input("날짜 범위", [min_log_dt, datetime.date.today()], key="lot_date")

    if not df_logs.empty:
        df_search = df_logs.copy()

        # 구분 필터
        if srch_type: df_search = df_search[df_search['구분'].isin(srch_type)]
        # 공장 필터
        if srch_fac != "전체": df_search = df_search[df_search['공장'] == srch_fac]
        # 날짜 필터
        if len(srch_date_range) == 2:
            s_d, e_d = srch_date_range
            df_search['날짜'] = pd.to_datetime(df_search['날짜'])
            df_search = df_search[(df_search['날짜'].dt.date >= s_d) & (df_search['날짜'].dt.date <= e_d)]
            df_search['날짜'] = df_search['날짜'].dt.strftime('%Y-%m-%d')
        # 키워드 필터
        if srch_keyword:
            kw = srch_keyword.strip()
            mask = (
                df_search['코드'].astype(str).str.contains(kw, case=False, na=False) |
                df_search['품목명'].astype(str).str.contains(kw, case=False, na=False) |
                df_search['비고'].astype(str).str.contains(kw, case=False, na=False)
            )
        ...
        if not df_search.empty:
            ...
            st.dataframe(df_search[safe_cols].sort_values(['날짜', '시간'], ascending=False), use_container_width=True, hide_index=True)
            ...
            # 품목별 집계
            if srch_keyword or len(srch_type) > 0:
                df_agg = df_search.groupby(['코드', '품목명', '구분'])['수량'].sum().reset_index()
